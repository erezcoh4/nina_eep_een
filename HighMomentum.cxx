...
// allow for ratio points to be calculated using cross sections event by event
...
// Proton/Neutron cross section parameters
...
void factor_cross_section_event_by_event( double * y_value , double * y_error );
...
double p_efficiency     ( double mom_GeV );                                             // Returns proton detection efficiency
double p_efficiency_err ( double mom_GeV );                                             // Returns proton detection efficiency error
...
// Erez edit July-26
int event_type; // 1=(e,e'p), 2=(e,e'p) after optimization, 3=(e,e'p_smeared), 4=(e,e'n)
...
int main(int argc, char ** argv) {
    ...
    // Erez edit July-26
    if (argc==2 && ((std::string)(argv[1])).find(".dat") != std::string::npos){
        print_error("you have included '.dat' extension of the input filename");
        return -1;
    }
    if (argc==3 && (atoi(argv[2])<0 || (atoi(argv[2])>1.e10))){
        print_error("you required an illegitimate number of events to process");
        return -1;
    }
	...
	double  Pmiss, Emiss, Mmiss, p_ecx, p_ecy, p_smeared, Pmiss_sm, Emiss_sm, Mmiss_sm, 
		...
		Z, N, theta_pq, p_over_q, psm_over_q, n_weight, p_weight;
	// --------------------
	// Target Mass
	double Mtar = -999.;
	...
	// --------------------------------------------------
    double Pmiss_low  = 0.30; // GeV   same
    double Pmiss_high = 1.00; // GeV   max on p miss, same
    double Mmiss_high = 1.10; // GeV   max on m miss, same
	double th_Nq_low  =  0.0; // deg
    double th_Nq_high = 25.0; // deg   theta pq, max 25 same cut
    double p_p_q_low  = 0.62; // GeV   same
	double p_p_q_high = 0.96; // GeV
    double n_p_q_low  = 0.62; // GeV   p/q
	double n_p_q_high = 1.10; // GeV
	// --------------------------------------------------
	// Optimization of QE cuts
	const int no_mmiss_cuts = 7;
	float Mmiss_cut_lim[no_mmiss_cuts] = {0};
    for(int i = 0 ; i < no_mmiss_cuts ; i++) Mmiss_cut_lim[i] = 1.101+0.037*(float)i; // at min_j = 2, this value is 1.175, cut for Mmiss

	const int no_pmiss_cuts = 25;
	float Pmiss_cut_lim[no_pmiss_cuts] = {0};
    for(int i = 0 ; i < no_pmiss_cuts ; i++) Pmiss_cut_lim[i] = 0.01+0.0245*(float)i; // at min_i = 16, this value is 0.402, cut for Pmiss
    ...
    // making a tree for the histogram dimensions
    // Erez edit July-26
    TFile ninasFile( "ninasFile.root", "recreate");
    TTree * ninasTree = new TTree( "ninaTree", "a Tree w data from dimensions");
    ninasTree -> Branch("Q2"         , &Q2          , "Q2/D"); // D for double
    ninasTree -> Branch("event_type" , &event_type  , "event_type/I"); // I for integer...
	...
	for(int evt=0;evt<nEnt_ep;evt++){   // loops through events
		if(evt%1000000==0) cout << " events processed = " << evt << " out of " << nEnt_ep << endl;
		T->GetEntry(evt);
		if(nProtons!=1) continue;
		if((Part_type[0]!=-11)||(Part_type[1]!=2212)){cout << "Error: event in (e,e'p) section doesn't have correct content of particles" << endl; exit(1);}
		...
		if((theta_pq < th_Nq_high )&&(theta_pq > th_Nq_low )&&(p_over_q > p_p_q_low)&&(p_over_q < p_p_q_high)){
			if((Pmiss>Pmiss_low)&&(Pmiss<Pmiss_high)){
				...
				if(Mmiss<Mmiss_high){
                    ...
                    // calculates the cross section ratios once per event
                    GKO5ratio = (sigma_p_GK05->Eval(Q2,theta_e))/(sigma_n_GK05->Eval(Q2,theta_e));
                    AMTratio = (sigma_p_AMT ->Eval(Q2,theta_e))/(sigma_n_AMT ->Eval(Q2,theta_e));
                    MMDratio = (sigma_p_MMD ->Eval(Q2,theta_e))/(sigma_n_MMD ->Eval(Q2,theta_e));
                    
                    nu_eep_een_GK05 += GKO5ratio;
                    nu_eep_een_AMT  += AMTratio;
                    nu_eep_een_MMD  += MMDratio;
    ...
    // ---------------------------------------------------------------
    // making a histogram of cross sections to verify similarity
    // TH1F * crossSections = new TH1F("Gaus", "sigma_(e,e'p)", 20, 1, 4 );
    //crossSections->GetXaxis()->SetTitle("sigma_(e,e'p)");
    //crossSections->GetYaxis()->SetTitle("Counts");
    // ---------------------------------------------------------------
	...
	for(int evt=0;evt<nEnt_ep;evt++){
        ...
        p_weight = 1./p_efficiency(h_mom.Rho());
		...
		if((theta_pq < th_Nq_high )&&(theta_pq >= th_Nq_low )&&(p_over_q > n_p_q_low)&&(p_over_q < n_p_q_high)){
			...
			if((Pmiss_sm>Pmiss_cut_lim[min_i])&&(Mmiss_sm<Mmiss_cut_lim[min_j])&&(Pmiss_sm<Pmiss_high)){
                ...
                if(p_points_to_EC_fid((q4+V4_Targ).Vect(),vtx_z[0],10.)){
                    GKO5ratio = sigma_p_GK05->Eval(Q2,theta_e);
                    AMTratio = sigma_p_AMT ->Eval(Q2,theta_e);
                    MMDratio = sigma_p_MMD ->Eval(Q2,theta_e);
                    local_average = ( GKO5ratio + AMTratio + MMDratio ) / 3;
                    nu_eep += p_weight/local_average;
                    raw_nu_p += 1.;
                    
                    // fill the histogram of cross sections
                    // crossSections -> Fill(local_average);
                }
	...
	for(int evt=0;evt<nEnt_ep;evt++){
        ...
        if(Xb<1.1) continue;    // this is the xB cut for neutrons specifically, greater than 1.1
		...
		if((theta_pq < th_Nq_high )&&(theta_pq > th_Nq_low )&&(p_over_q > n_p_q_low)&&(p_over_q < n_p_q_high)){  // this is the cut for theta_pq, 0 to 25, also p_over_q, 0.62 to 1.10
			...
			if((Pmiss>Pmiss_cut_lim[min_i])&&(Mmiss<Mmiss_cut_lim[min_j])&&(Pmiss<Pmiss_high)){
				...
				if(p_points_to_EC_fid((q4+V4_Targ).Vect(),vtx_z[0],10.)){
                    GKO5ratio = sigma_n_GK05->Eval(Q2,theta_e);
                    AMTratio = sigma_n_AMT ->Eval(Q2,theta_e);
                    MMDratio = sigma_n_MMD ->Eval(Q2,theta_e);
                    local_average = ( GKO5ratio + AMTratio + MMDratio ) / 3;
					raw_nu_n += 1.;
					nu_een   += n_weight/local_average;
				}
			}
		}
        // Erez edit July-26
    ...
    // Erez edit July-26
    cout << "writing output tree of "<< ninasTree->GetEntries() << " events into " << ninasFile.GetName() << endl;
    ninasFile.cd();
    ninasTree -> Write();
   ...
   // factor_cross_section_event_by_event( g_model_ratio_y , g_model_ratio_e );
	...
	g_model_ratio -> SetMinimum(0);
	g_model_ratio -> SetMaximum(6);
	pretty_TGraphErrors(g_model_ratio);
	g_model_ratio -> Draw("AE3");

    // calculate uncertainty and ratios
    
	double A_x = N+Z;
	double eep_een      = (nu_eep)/(nu_een);
	//double e_sta_eep_een = sqrt( pow(sqrt(nu_eep)/raw_nu_n,2) + pow(nu_eep*sqrt(nu_een)/pow(nu_een,2),2) );       // statistical error
    double e_sta_eep_een = sqrt(pow(1/sqrt(raw_nu_p),2)+pow(1/sqrt(raw_nu_n),2));                              // my statistical error
    //double e_def_eep_een = 0; //sqrt( pow(n_eff_p0*n_eff_m_e,2)+pow(n_eff_p0_e*n_eff_m,2) );                             // neutron detection eff error
    double e_def_eep_een = n_efficiency_err( h_mom.Rho());                                                      // n detection error
    double p_det_err = p_efficiency_err(h_mom.Rho());                                                           // p detection error
    ...
    cout << "\np det eff   error: " << p_det_err << endl;
	cout << "**************" << endl;
    
    cout << "neutron detection efficiency: " << n_efficiency(h_mom.Rho()) << endl;
    cout << "proton detection efficiency: " << p_efficiency(h_mom.Rho()) << endl;

	cout << "Number of protons  contributing to this ratio: " << raw_nu_p * p_weight << endl;
	cout << "Number of neutrons contributing to this ratio: " << raw_nu_n * n_weight << endl << endl;
    cout << "raw nu p: " << raw_nu_p << endl;
    cout << "raw nu n: " << raw_nu_n << endl;
    cout << "nu_een: " << nu_een << endl;
    cout << "nu_eep: " << nu_eep << endl << endl;
...
// ====================================================================================================================================================
void factor_cross_section_event_by_event( double * y_value , double * y_error , vector<double>& sigmas ){
...
// ====================================================================================================================================================
// for now, we are just estimating with 50% efficiency
double p_efficiency     ( double mom_GeV ) {
    return 0.50;
}
// ====================================================================================================================================================
// for now, we are just estimating with 1% uncertainty
double p_efficiency_err ( double mom_GeV ) {
    return 0.01;
}
// ====================================================================================================================================================
